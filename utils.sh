...
pd login ubuntu --user $varname --shared-tmp -- env DISPLAY="$DISPLAY_NUM" "$@"
...
if [[ $action == "Copy .desktop file" ]]; then
  ...
  sed -i "s/^Exec=\(.*\)$/Exec=pd login ubuntu --user $username --shared-tmp -- env DISPLAY=$DISPLAY_NUM \1/" "$PREFIX/share/applications/$desktop_filename"
...
REPO_URL="https://github.com/brfluidpri/App-Installer.git"
